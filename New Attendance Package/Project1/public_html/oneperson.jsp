...
<html>
  ...
  function tableToExcel(){
       ...
       // return function (table) {
          //  if (!table.nodeType) table = document.getElementById('printableArea')
            var ctx = { worksheet: 'Applications' || 'Worksheet', table: document.getElementById('printableArea').innerHTML }
...
year_value = current_date.getFullYear();
    ...
    var filestring=day_value+months[month_value]+year_value;
...
%>
   ...
   <input type="hidden" id="tofl" name="to" value="<%=session.getAttribute("todate").toString()%>"/>
      ...
      <thead>   STATEMENT SHOWING LATES IN WORKING DAYS FOR THE MONTHS BETWEEN <%=session.getAttribute("fromdate").toString()%>
   ...
   String emptemp=session.getAttribute("empid").toString();
        Query2=Query2+" where TRIM(EMP_NO)='"+emptemp+"' " ;
        
        
           Query2=Query2+"  and to_char(ATTENDANCE_DATE,'yyyy-mm-dd') between '"+fromdate+"' and '"+todate+"' ";
       Query2=Query2+"  ORDER BY ATTENDANCE_DATE DESC";
    ...
    catch(Exception ex)
    {
                ...
                //employee only data
                String empselect2=session.getAttribute("empselect").toString();
...
<input type="hidden" id="checkedin" value="<%=session.getAttribute("check")%>"/>
    ...
    q="SELECT distinct EMP_NO,Employee_Name,Designation,Branch_N,exst, COUNT(*) from";
           ...
           q=q+" where  to_char(ATTENDANCE_DATE,'yyyy-mm-dd') between '"+fro+"' and '"+top+"'";
            q=q+"  and TRIM(ed.EMP_NO)='"+emp_id+"') GROUP BY EMP_NO,Employee_Name,Designation,Branch_N,exst order by EMP_NO";
    ...
    String from = session.getAttribute("fromdate").toString();
                 String to =session.getAttribute("todate").toString();
                  String q7="select DISTINCT to_char( attendance_date,'yyyy-mm-dd') dater from EMPLOYEE_ATTENDANCE   where  "; 
        q7=q7+"to_char( ATTENDANCE_DATE,'yyyy-mm-dd') BETWEEN '"+from+"' and '"+to+"' "; 
        // end of employee only data
             
             q7=q7+"  and TRIM( EMP_NO)='"+emp_id+"'";
            // end of employee only data
        q7=q7+" order by dater desc" ;    
...
<a id="exporter" name="exporter" href="#" onclick="tableToExcel()">Export to Excel</a>
